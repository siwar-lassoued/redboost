    ...
    <div class="p-6 bg-[#F8FAFC] min-h-screen">
      ...
      <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex items-center gap-2 px-5 py-2.5 bg-[#1A3A3A] text-white rounded-full text-xs font-black shadow-lg shadow-[#1A3A3A]/20">
          <div class="w-1.5 h-1.5 rounded-full bg-[#3aafff] animate-pulse"></div>
          {{ progress() }}% COMPLÉTÉ
        </div>
      </div>
      ...
      <!-- Main Grid: Coach · Urgent Tasks · Next Session -->
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-3xl p-6 shadow-xl shadow-gray-200/50 border border-gray-100">
          ...
          @if (assignedCoach()) {
            ...
            <!-- Stars (Placeholder for now, but name/specialty are dynamic) -->
            ...
            <div class="flex flex-wrap gap-1.5 mb-5">
              @for (tag of coachTags(); track tag) {
                <span class="text-[10px] px-2.5 py-1 rounded-full font-black uppercase tracking-widest"
                  style="background: #e8f9ff; color: #3aafff">{{ tag }}</span>
              }
            </div>

              <div class="flex gap-2">
                <button routerLink="/entrepreneur/mes-coachs"
                  class="flex-1 py-3 rounded-2xl text-sm font-black text-white transition-all hover:scale-[1.02] shadow-lg shadow-[#ff3d91]/20"
                  style="background: linear-gradient(135deg, #1A3A3A, #3aafff)">
                  Profil complet
                </button>
                <button [routerLink]="['/entrepreneur/chat']" [queryParams]="{with: assignedCoach()!.id}"
                  class="w-14 py-3 rounded-2xl flex items-center justify-center text-white transition-all hover:scale-[1.05] shadow-lg shadow-[#ff3d91]/20"
        ...
        <div class="bg-white rounded-3xl p-6 shadow-xl shadow-gray-200/50 border border-gray-100">
          <div class="flex items-center justify-between mb-5">
            <h3 class="text-lg font-black text-[#1A1A2E] flex items-center gap-2">
              <i class="pi pi-exclamation-triangle text-[#ff3d91]"></i>
              Tâches Urgentes
            </h3>
            <button routerLink="/entrepreneur/mes-taches" class="text-xs font-black text-gray-400 hover:text-[#ff3d91] transition-colors uppercase tracking-widest">Voir tout</button>
          </div>
            ...
            <div class="space-y-3">
              @for (task of urgentTasks(); track task.id) {
                <div class="flex items-center gap-3 p-4 rounded-2xl border"
                  style="background: #fff0f5; border-color: rgba(255,61,145,0.15)">
                  <i class="pi pi-exclamation-triangle flex-shrink-0 text-[#ff3d91]"></i>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-xl shadow-gray-200/50 border border-gray-100">
          <h3 class="text-lg font-black text-[#1A1A2E] mb-5 flex items-center gap-2">
            <i class="pi pi-calendar text-[#a17dfd]"></i>
            Prochaine Session
          </h3>

          @if (nextSession()) {
            <!-- Session gradient card -->
            <div class="rounded-2xl p-5 mb-4 text-white" style="background: linear-gradient(135deg, #1A3A3A 0%, #3aafff 100%)">
            ...
            @if (nextSession()!.meetLink) {
              <a [href]="nextSession()!.meetLink" target="_blank"
                class="w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-sm font-black text-white bg-sky-500 hover:bg-sky-600 transition-colors shadow-lg shadow-sky-500/20">
            ...
            <div class="py-12 text-center">
              ...
              <button routerLink="/entrepreneur/mes-sessions"
                class="flex items-center gap-1.5 text-sm font-black text-[#ff3d91] mx-auto hover:underline uppercase tracking-widest">
  ...
  kpiCards = computed(() => {
    ...
    return [
      { 
        ...
        icon: 'user-edit', 
        gradient: 'linear-gradient(135deg, #ff3d91 0%, #a17dfd 100%)', 
        shadow: '0 4px 16px rgba(255,61,145,0.30)' 
      },
      { label: 'SESSION', value: this.nextSession() ? 'OUI' : 'NON', subtext: 'prochaine', footer: this.nextSession() ? 'Planifiée' : 'Aucune', icon: 'calendar', gradient: 'linear-gradient(135deg, #a17dfd 0%, #7B52D3 100%)', shadow: '0 4px 16px rgba(161,125,253,0.30)' },
      { label: 'TÂCHES', value: this.totalTasks().toString(), subtext: 'à faire', footer: 'En cours', icon: 'list', gradient: 'linear-gradient(135deg, #1A3A3A 0%, #3aafff 100%)', shadow: '0 4px 16px rgba(58,175,255,0.25)' },
      { label: 'PROGRESSION', value: this.progress() + '%', subtext: 'globale', footer: 'Performance', icon: 'chart-line', gradient: 'linear-gradient(135deg, #FF6F00 0%, #C0392B 100%)', shadow: '0 4px 16px rgba(192,57,43,0.25)' },
    ];
  });
}
