// admin_event_calendar.ts
import { Component, OnInit } from '@angular/core';
import { MatDialog, MatDialogModule } from '@angular/material/dialog';
import { AddEventDialogComponent } from '../../event_organizer/dialogs/add_event_dialog/add_event_dialog';
...
import { EventService, EventResponse } from '../../event_organizer/event.service';
import { TypeFormationService, TypeFormation } from '../../event_organizer/type-formation.service';
...
@Component({
  ...
  templateUrl: './admin_event_calendar.html',
  styleUrls: ['./admin_event_calendar.scss'],
  standalone: true,
  imports: [
    CommonModule,
    MatIconModule,
...
deleteEvent(event: CalendarEvent): void {
  ...
  getEventTypeIconPrime(typeName: string): string {
    ...
    if (lower.includes('pitch')) return 'pi-bullseye';
    if (lower.includes('networking')) return 'pi-users';
    if (lower.includes('formation')) return 'pi-book';
    if (lower.includes('atelier')) return 'pi-wrench';
    if (lower.includes('célébration') || lower.includes('celebration')) return 'pi-star';
    if (lower.includes('présentation') || lower.includes('presentation')) return 'pi-desktop';
    return 'pi-calendar';
  }

  getModeIconPrime(mode: string): string {
    if (mode === 'virtuel') return 'pi-video';
    if (mode === 'en-personne') return 'pi-map-marker';
    return 'pi-users';
  }

  openGoogleCalendar(event: CalendarEvent): void {
    if (event.meetLink) {
      // If it has a meet link, it might be synced, but let's just create a new one anyway
    }
    const [startTime, endTime] = event.time.split(' - ');
    ...
    const startIso = d.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '');
    ...
    const url = `https://calendar.google.com/calendar/r/eventedit?text=${title}&dates=${startIso}/${endIso}&details=${details}&location=${location}`;
